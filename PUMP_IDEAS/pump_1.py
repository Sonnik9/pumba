...
# Июн 7, 2021
# Информация о релизе:
# - Added MA filter option for dump detector.
# With the filter on, only dumps in uptrend are detected. Removing filter will show all dump signals and may be a "spammy"
# Июн 8, 2021
# Информация о релизе:
# Added alerts for buy point and sell point
# Июл 7, 2021
# Информация о релизе:
# Added some cosmetic changes. 2 new themes: Large and Tartan or as I like to call it... "Scotch"astic.
# Large is now set to default as it is cleaner and easier to read than original. Signals can be turned on in settings.
# snapshot
# Июл 20, 2021
# Информация о релизе:
# Boom Hunter now has a new theme and default look using classic trading view colors.
...
# study(title="Boom Hunter", shorttitle="Boom", overlay=false)
